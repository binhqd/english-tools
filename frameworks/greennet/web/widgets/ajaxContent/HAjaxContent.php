...
class HAjaxContent extends CWidget
{
	...
	{
		if ($this->url == null)
			throw new CException('The property "url" cannot be empty.');

		if ($this->supportLoadMore !== false) {
			if (!isset($this->supportLoadMore['linkPager']))
				throw new CException('The property "supportLoadMore.linkPager" cannot be empty.');
			if (!isset($this->supportLoadMore['loadingElementClass']))
				throw new CException('The property "supportLoadMore.loadingElementClass" cannot be empty.');
		}
