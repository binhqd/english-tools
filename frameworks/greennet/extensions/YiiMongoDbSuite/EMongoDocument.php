...
abstract class EMongoDocument extends EMongoEmbeddedDocument
{
	...
	public function findAllAndGroup($arrGroup, $criteria=null)
	{
...
// 				$cursor->fields($criteria->getSelect(true));
			...
			$cursor = $this->getCollection()->group($keys, $initial, $reduce, $criteria->getConditions());
			
			// Use insertion sort
			for ($k = 0; $k < count($cursor['retval']); $k++) {
				for ($i = 1; $i < count($cursor['retval'][$k]['items']) - 1; $i++) {
					
					$item = $cursor['retval'][$k]['items'][$i];
					
					$j = $i;
					$itemAt = $cursor['retval'][$k]['items'][$j - 1];
					if (empty($item['rating']))
						$item['rating'] = array(
						'average'	=> 0,
						'count'		=> 0
						);
					if (empty($itemAt['rating']))
						$itemAt['rating'] = array(
							'average'	=> 0,
							'count'		=> 0
						);
					...
					while ($j > 0 
							&& (
								$itemAt['rating']['average'] < $item['rating']['average'] 
								|| (
									$itemAt['rating']['average'] == $item['rating']['average']
									&& $itemAt['rating']['count'] < $item['rating']['count']
								)
							)
					) {
						$cursor['retval'][$k]['items'][$j] = $cursor['retval'][$k]['items'][$j - 1];
						$j--;
					}
					$cursor['retval'][$k]['items'][$j] = $item;
				}
			}
			//$cursor = $this->getCollection()->find($criteria->getConditions());
			return $this->populateGroupRecords($cursor, $criteria);
		}
		return array();
	}
	/**
	...
	public function populateGroupRecords($groups, $criteria = null) {
		
		$ret = array();
		if (isset($groups['retval'])) {
			foreach ($groups['retval'] as $group) {
				$ret[$group["biz_id"]] = $this->populateRecords($group["items"]);
			}
		}
