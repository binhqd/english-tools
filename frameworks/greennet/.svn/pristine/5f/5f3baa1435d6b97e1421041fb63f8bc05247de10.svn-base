	...
	*/
	public function run($email)
	{
		...
		if (GNTmpUser::model()->findByEmail($email)) {
			...
			$strMessage = UsersModule::t('Your email address has registered on our system.<br/>'.
			...
			if ($controller->isJsonRequest) {
				...
				Yii::app()->jlbd->dialog->confirm(UsersModule::t('GreenNet Message!'), $strMessage, 'function(reply){
					if (reply) {
		...
		if (GNUser::model()->findByEmail($email)) {
			...
			$message = UsersModule::t('Your email <b>{email}</b> has been used in GreenNet system.<br/>Do you want to <a title="Login" href="{strUrlLogin}">login</a> with this email?', array('{strUrlLogin}'=>$strUrl, '{email}'=>$email));
			// Confirm not OK. :D
			if(yii::app()->request->isAjaxRequest) {
				...
				Yii::app()->jlbd->dialog->confirm(UsersModule::t('GreenNet Message!'), $message, 'function(reply){
					if (reply) {
						jlbd.redirect("'.$strUrl.'");
					}
				}');
