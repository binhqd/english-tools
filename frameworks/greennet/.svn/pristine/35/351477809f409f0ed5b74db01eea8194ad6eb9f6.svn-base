...
class GNDeleteArticleAction extends GNAction {
	...
	public function run()
	{
		...
		try {
			...
			if (empty($model)) {
				$strMessage = ArticleMessage::t("The system can't perform your request because of invalid ID");
				$url = $this->errorUrl;
			...
			$strMessage = ArticleMessage::t("Article has been deleted successful");
