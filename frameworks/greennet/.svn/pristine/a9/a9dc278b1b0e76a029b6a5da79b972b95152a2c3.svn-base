...
class InviteController extends GNController
{
	...
	public function actionActive($key = NULL, $email = NULL){
		if(!isset($key) || !isset($email)) {
			Yii::app()->user->setFlash('warning', 'Code or email is invalid.');
			$this->redirect('/');
		}
		...
		if(!$checkCode) {
			Yii::app()->user->setFlash('warning', 'Code is expired or invalid.');
			$this->redirect('/');
		}
		...
		if(!empty($activeEmail)){
			...
			if(!empty($currentUser)){
				if(currentUser()->isGuest) {
					Yii::app()->user->setFlash('warning', 'This email has been existed in system. Please login to use!');
				}
	...
	public function actionIndex() {
		if(!empty($_POST)){
			...
			if(!empty($existEmail)) {
				jsonOut(array(
					'error'		=> true,
					'message'	=> 'Email has been exist in system.'
				));
			}
			...
			if (!$saveActive) {
				...
				$msg = 'User has been invited to join Slidelane';
			}
