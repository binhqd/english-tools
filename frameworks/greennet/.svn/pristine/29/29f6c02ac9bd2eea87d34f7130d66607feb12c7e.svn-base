	...
	public function run() {
		...
		if (!empty($_POST['GNUser']) || !empty($_POST['GNUserProfile'])) {
			...
			if (empty($userProfile)) { // User does not profile => create profile
				...
				if (!$createProfile) throw new Exception('Cannot create user profile');
			}
			...
			if ($valid) {
				...
				try {
					...
					if (!$user->save()) throw new Exception('Cannot save user information');
					...
					if (!$userProfile->save()) throw new Exception('Cannot save user profile information');
					...
					if ($controller->isJsonRequest) {
						...
						Yii::app()->jlbd->dialog->notify(array(
							'type'		=> 'success',
							'autoHide'	=> true,
							'message'	=> UsersModule::t('Your profile has been saved sucessful'),
						));
					}
				} catch (Exception $ex) {
