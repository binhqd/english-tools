		...
		if (!isset($_GET['code'])) {
			if (!$this->isJsonRequest) {
				Yii::app()->jlbd->dialog->notify(array(
					...
					'type' => 'error',
					'autoHide' => true,
					'message' => UsersModule::t('Could not login with Google. Because parameter is invalid.'),
				));
				$this->redirect('/');
			} else {
				ajaxOut(array(
		...
		try {
			...
			if ($isConnected) {
				...
				if (empty($user)) {
					...
					$sendMail = Yii::app()->mail->sendMailWithTemplate($user->email, UsersModule::t('GreenNet membership created'), 'sendMailUserSocialCreated', $data=array('user'=>$user, 'password'=>$password, 'socialName'=>'Google'));
				}
				...
				if (!$this->isJsonRequest) {
					Yii::app()->jlbd->dialog->notify(array(
						...
						'type' => 'error',
						'autoHide' => true,
						'message' => UsersModule::t('Could not login with Google.'),
					));
					$this->redirect('/');
				} else {
					ajaxOut(array(
			...
			if (!$this->isJsonRequest) {
				Yii::app()->jlbd->dialog->notify(array(
					...
					'type' => 'error',
					'autoHide' => true,
					'message' => UsersModule::t($e->getMessage()),
				));
				$this->redirect('/');
			} else {
				ajaxOut(array(
