				...
				try {
					...
					if ($modelTmpUser->createUser($model->attributes)) {
						...
						if ($controller->isJsonRequest) {
							...
							$sendMail = Yii::app()->mail->sendMailWithTemplate($modelTmpUser->email, UsersModule::t('GreenNet membership confirmation'), 'sendMailActivationAccount',$data=array('strActiveUrl'=>$strActiveUrl, 'user'=>$modelTmpUser));
