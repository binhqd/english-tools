	...
	public function run()
	{
		...
		if (!empty($_POST['GNChangeEmailForm'])) {
			...
			try {
				...
				$sendTo		= $model->email;
				$subject	= 'Confirm email ownership';
				$message	= $link;
				...
				$msg = 'The system has sent an email to '.$model->email.' for confirming about email ownership.<br/>Please check '.$model->email.' to continue.';
