	...
	public function actionIndex() {
		if (!$this->_connector->isConnected) {
			...
			if (!empty($user)) {
				...
				// Set success flash
				$msg = "You are now successful login";
				...
				throw new Exception("This email ({$userInfo['email']}) doesn't exist in our database");
			}
		...
		if (!isset($_GET['code'])) {
			if (!$this->isJsonRequest) {
				Yii::app()->jlbd->dialog->notify(array(
					...
					'type' => 'error',
					'autoHide' => true,
					'message' => UsersModule::t('Could not login with Facebook. Because parameter is invalid.'),
				));
				$this->redirect('/');
			} else {
				ajaxOut(array(
		...
		if ($this->_connector->isConnected) {
			...
			if (empty($user)) {
				...
				$arrInfo = array(
					...
					'username'	=> GNUser::createDisplayName($firstname, $lastname)
				);
				...
				$sendMail = Yii::app()->mail->sendMailWithTemplate($user->email, UsersModule::t('GreenNet membership created'), 'sendMailUserSocialCreated', $data=array('user'=>$user, 'password'=>$password, 'socialName'=>'Facebook'));
			}
			...
			if (!$this->isJsonRequest) {
				...
				Yii::app()->jlbd->dialog->notify(array(
					...
					'type' => 'success',
					'autoHide' => true,
					'message' => UsersModule::t('Login successful.'),
				));
				$this->redirect('/');
			...
			if (!$this->isJsonRequest) {
				Yii::app()->jlbd->dialog->notify(array(
					...
					'type' => 'error',
					'autoHide' => true,
					'message' => UsersModule::t('Could not login with Facebook.'),
				));
				$this->redirect('/');
			} else {
				ajaxOut(array(
...
// 		));
	...
	public function actionSyncPhotos() {
		...
		if (!$profilePhoto->isConnected) {
			...
			$photos = $profilePhoto->getPhotos(0, 20);
...
// 			dump(currentUser()->id);
			...
			$records = GNSyncFacebookPhoto::model()->findAll('user_id=:user_id AND done=0', array(
				':user_id'	=> currentUser()->id
			));
	...
	private function _downloadAvatar($strUrlImage, $strUserId)
	{
		...
		try {
			...
			@mkdir(Yii::getPathOfAlias('jlwebroot') . '/' . "upload/user-photos/{$strUserId}", 0777, true);
	...
	public function actionGetPhoto() {
		...
		if (!empty($imageSize)) {
			...
			$out = array(
				'error'		=> true,
				'message'	=> "Can't get imagesize of {$record}"
			);
			ajaxOut($out);
		}
