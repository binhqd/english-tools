...
class GNRatingObject extends GNActiveRecord
{
	...
	public function rate($binObjectId, $binUserId, $binRatingValueId, $intRateValue) {
		...
		if (!isset($binObjectId) || !isset($binUserId) || !isset($binRatingValueId) ||!isset($intRateValue)) {
			throw new Exception("Can't rate for object, because params is invalid.");
		}
		...
		if (empty($modelRatingValueData)) {
			throw new Exception("Can't rate for object, because rating value is empty.");
		}
		...
		if (empty($hasRate)) {
			...
			if (!$modelRatingObjectNew->validate()) {
				throw new Exception("Can't rate for object, because data attibutes is not validate.");
			}
			...
			if(empty($err_saveRate)){
				throw new Exception("Can't save rate for object. Because error by system.");
			} else {
				list($field, $_err) = each($err_saveRate);
				throw new Exception("Can't save rate for object. Because errors is : ".$_err[0]);
			}
		} else {
	...
	public function updateValueRateObject($binRatingObject, $binRatingValueId, $intRateValue) {
		...
		if (!isset($binRatingObject) || !isset($intRateValue) || !isset($binRatingValueId)) {
			throw new Exception("Can't update value rate for object. Because data attibutes is not validate.");
		}
		
		$modelRatingObjectData = $modelRatingObject::model()->findByPk($binRatingObject);
		
		if (empty($modelRatingObjectData)) {
			throw new Exception("Can't update value rate for object. Because rating object id is invalid.");
		}
		...
		if(empty($err_updateRate)){
			throw new Exception("Can't update value rate for object. Because error by system.");
		} else {
			list($field, $_err) = each($err_updateRate);
			throw new Exception("Can't update value rate for object. Because errors is : ".$_err[0]);
		}
	}
	...
	public function getMyRating($strRatingAlias, $binUserId, $limit = null, $offset = 0) {
		...
		if(!isset($strRatingAlias) || !isset($binUserId)) {
			throw new Exception("Can\'t get my rating. Because params is invalid"); 
		}

		$modelRatingData = $modelRating::model()->getRatingsByAlias($strRatingAlias);
		
		if (empty($modelRatingData)) {
			throw new Exception("Can\'t get my rating. Because alias rating is empty");
		}
	...
	public function getCountMyRating($strRatingAlias, $binUserId) {
		...
		if(!isset($strRatingAlias) || !isset($binUserId)) {
			throw new Exception("Can\'t get count my rating. Because params is invalid");
		}
