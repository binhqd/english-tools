...
class GNUploadAction extends CAction {
	...
	public function run() {
		...
		//$this->sendHeaders();
		$config = array(
			'class'			=> 'greennet.components.GNSingleImage.components.GNSingleUploadImage',
		...
		if (!empty($image)) {			
			...
			$this->_model->image = $image['filename'];
		...
		$return = array('files' => array(array(
			...
			"thumbnail_url"	=> GNRouter::createUrl("/{$this->_uploadPath}/fill/150-150/{$image['filename']}"),
