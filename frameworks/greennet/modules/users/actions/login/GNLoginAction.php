		...
		if (currentUser()->id != -1) {
			if ($controller->isJsonRequest)
				ajaxOut(array(
					...
					'message'	=> UsersModule::t('You already logged in'),
				));
			else
				$controller->redirect('/');
		}
	...
	// 					unset(Yii::app()->session['emailRequestLogin']);
						if ($controller->isJsonRequest) {
							ajaxOut(array(
								'error'	=> false,
								'code'	=> self::TYPE_LOGIN_SUCCESS,
								'message'	=> UsersModule::t('You are successfully logged in'),
								'url'	=> $url
							));
						} else {
					...
					if ($controller->isJsonRequest) {
						ajaxOut(array(
							...
							'message'	=> UsersModule::t("Your email or password is invalid"),
						));
					} else {
						Yii::app()->jlbd->dialog->notify(array(
							'error'	=> true,
