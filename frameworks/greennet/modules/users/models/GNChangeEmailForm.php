	...
	public function checkEmail($attribute, $params)
	{
		if(!$this->hasErrors())
		{
			if ($this->$attribute == currentUser()->email) {
				$this->addError($attribute, "Your email currently is {$this->email}. You need to enter another email.");
			} else {
				...
				if (!empty($checkEmailUser))
					$this->addError($attribute, 'This email has already existed in system');
			}
		}
	}
