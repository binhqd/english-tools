...
class GNCoreUser extends GNActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('email', 'email'),
			array('email', 'unique', 'message' => UsersModule::t("Your email has been already exists in system.")),
...
// 			),
	...
	public function attributeLabels()
	{
		return array(
			...
			'displayname' => UsersModule::t('Display Name'),
			'password' => UsersModule::t('Password'),
			'email' => UsersModule::t('Email'),
			'created' => UsersModule::t('Date Join'),
			'lastvisited' => UsersModule::t('Last Visited'),
...
// 						dump($user);
	...
	public function createUser($arrInformation, $className=__CLASS__) {
		...
		if (empty($arrInformation['firstname']) && empty($arrInformation['lastname'])) {
			throw new Exception("Firstname & Lastname can't be both left empty");
		}
	...
	public static function createDisplayName($firstname, $lastname) {
		return ucfirst(strtolower($firstname)) . " " . strtoupper(substr($lastname, 0, 1)) . ".";
	}
	...
	{
		if ($intLength < 1) throw new InvalidArgumentException(self::t('Minimum length is 1'));
		if ($intLength > 32) throw new InvalidArgumentException(self::t('Maximum length is 32'));
...
// 			dump('from cache');
	...
	public function setAvatar($filename) {
		...
		if (!empty($profile)) {
			...
			watch("Set avatar for {$this->displayname} to {$filename}. <a href='".GNRouter::createAbsoluteUrl("/upload/user-photos/{$this->hexID}/{$filename}")."'>Link</a>");
		} else {
