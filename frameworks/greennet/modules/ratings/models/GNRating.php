...
class GNRating extends GNActiveRecord
{
	...
	public function createRating($arrAttributes) {
		...
		if (empty($arrAttributes)) {
			throw new Exception("Can't create rating. Because params attibutes is invalid.");
		}
...
// 		$modelRating = new self();
		...
		if (!$modelRating->validate()) {
			throw new Exception("Can't create rating. Because data attibutes is not validate.");
		}
		...
		if(empty($err_saverating)){
			throw new Exception("Can't create rating. Because error by system.");
		} else {
			list($field, $_err) = each($err_saverating);
		
			throw new Exception("Can't create rating. Because errors is : ".$_err[0]);
		}
	}
	...
	public function removeRating($binRatingsId) {
		...
		try {
			...
			if (!$removeAllRatingValue) {
				throw new Exception("Can\'t remove rating. Because can\'t remove all rating value.");
			}
			
			$modelRatingData = $modelRating::model()->findByPk($binRatingsId);
			
			if (empty($modelRatingData)) {
				throw new Exception("Can\'t remove rating. Because model rating is empty.");
			}
			...
			if(empty($err_removeRating)){
				...
				throw new Exception("Can\'t remove rating. Because error by system.");
				
			} else {
				list($field, $_err) = each($err_removeRating);
			
				throw new Exception("Can\'t remove rating. Because errors is : ".$_err[0]);
			}
		} catch (Exception $e) {
