...
class GNController extends RController {
	...
	public function init() {
		...
		if (isset($_COOKIE['notificationClicked'])) {
			$modifier = new EMongoModifier();
			// replace field1 value with 'new value'
			$modifier->addModifier('isClicked', 'set', 1);
			$modifier->addModifier('read', 'set', 1);
			
			// prepare search to find documents
			$criteria = new EMongoCriteria();
			$criteria->addCond('_id','==', new MongoID($_COOKIE['notificationClicked']));
			$criteria->addCond('receiver_id','==', currentUser()->hexID);
			
			
			// update all matched documents using the modifiers
			Yii::import('application.components.notification.JLNotificationDocument');
			$status = JLNotificationDocument::model()->updateAll($modifier, $criteria);
		...
		var notificationUrl = \"".Yii::app()->params['notificationServer']."\";
		".(isset($_COOKIE['notificationClicked']) ? "$.cookies.del('notificationClicked');" : "")."
		...
		if (!currentUser()->isGuest) {
			...
			$arrUser = currentUser()->safeAttributes;
		} else {
			$arrUser = array(
				'id' => '-1',
				'displayname' => 'Guest',
		...
		/** dunghd add code for get location **/
		// 5/4/2012
        /*
		$geoInfo = Yii::app()->cache->get('geoInfo.user-' . Yii::app()->user->id . '.ip.' . $_SERVER['REMOTE_ADDR']);
		...
		if($geoInfo===false)
		{
			if ( function_exists('curl_init') ) {		
				...
				Yii::app()->cache->set('geoInfo.user-' . Yii::app()->user->id . '.ip.' . $_SERVER['REMOTE_ADDR'],unserialize($geoInfo));
				curl_close ($ch);
				
			} else if ( ini_get('allow_url_fopen') ) {				
				$geoInfo = file_get_contents('http://www.geoplugin.net/php.gp', 'r');
				Yii::app()->cache->set('geoInfo.user-' . Yii::app()->user->id . '.ip.' . $_SERVER['REMOTE_ADDR'],unserialize($geoInfo));
			}
		}
		
		$userLocation = '';
		if($geoInfo['geoplugin_countryCode'] == 'AU')
		{
			if($userLocation != '')
			{
				$userLocation = $geoInfo['geoplugin_city'] . ',' . $geoInfo['geoplugin_countryName'];
			}
			else
			{
				$userLocation = $geoInfo['geoplugin_countryName'];
			}
		}
		else
		{
		...
		Yii::app()->cache->set('userLocation.user-' . Yii::app()->user->id ,$userLocation);
		*/
