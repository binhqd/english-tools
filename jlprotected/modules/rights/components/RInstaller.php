...
class RInstaller extends CApplicationComponent
{
	...
	*/
	public function run()
	{
        ...
        // HuyTBT add constraint authassignment with users table
        $schema[] = 'ALTER TABLE `'.$assignmentTable.'` ADD CONSTRAINT `fk_authassignment_users` FOREIGN KEY (`userid`) REFERENCES `'.Yii::app()->getUser()->model->tableName().'` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION';
