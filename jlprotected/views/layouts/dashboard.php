...
// 	GNAssetHelper::cssFile('login-page');
	...
	GNAssetHelper::scriptFile('jquery.rating', CClientScript::POS_HEAD);
	...
	GNAssetHelper::scriptFile('autoresize.jquery', CClientScript::POS_HEAD);
	...
	GNAssetHelper::scriptFile('jlbd.layout-dashboard', CClientScript::POS_HEAD);
	GNAssetHelper::scriptFile('jlbd.share', CClientScript::POS_END);
	?>
	...
	GNAssetHelper::scriptFile('jlbd.biz', CClientScript::POS_HEAD);
	?>
	...
	//GNAssetHelper::setBase('application.modules.lists.assets', 'lists');
	//GNAssetHelper::scriptFile('jlbd.add-list', CClientScript::POS_END);
	//GNAssetHelper::scriptFile('jlbd.add-list.menu', CClientScript::POS_END);
	
	GNAssetHelper::setBase('widgets.photos.assets');
	GNAssetHelper::scriptFile('jlbd.photo', CClientScript::POS_END);
...
<div class='hidden' style='display:none'>
	<?php
		$this->prototypeWidget('widgets.review.JLBDReviewForm', array(
			//'strBizID' => $biz['attrs']['uuid'],
			'isShowLink' => false,
			//'intYourRate' => $userRates[$index],
		));
		$this->prototypeWidget('widgets.comment.JLBDCommentBox', array());
		$this->prototypeWidget('widgets.comment.JLBDCommentItemsJS', array());
		$this->prototypeWidget('widgets.compliment.JLBDComplimentBox', array());
	?>
	<?php $this->prototypeWidget('widgets.business.JLBDBusinessItemViewMode'); ?>
	<?php $this->prototypeWidget('widgets.review.JLBDReviewsByBusinessPrototype'); ?>
</div>

<?php $this->prototypeWidget('widgets.favourite.JLBDFavouritePopup', array()); ?>
<?php $this->prototypeWidget('widgets.friends.JLBDFriendsSelectPopup'); ?>

<?php $this->renderPartial('//common/jlap-rating');?>
<?php $this->renderPartial('//common/snapshot');?>
			...
			<div class="wd-center">
				...
				if (!currentUser()->isGuest) {
						...
						$this->widget('widgets.multi-menu.JLBDUserMenu', array(
							...
							'arrMenu' => $arrMenu,
							'binUserID' => currentUser()->id // hoặc có thể sử dụng 'user_id'	=> '' 
						));	
				}
				?>
			</div>
...
window.onload = function(){setTimeout(function(){
	var lnk = $('.accordionion ul li.wd-send-message a');
	lnk.die();
	lnk.off();
	var hf = lnk.attr('href');
	lnk.click(function(e){
		e.stopPropagation();
		setTimeout(function(){location.href=hf}, 200);
		return false;
	})
}, 2000)}
</script>
